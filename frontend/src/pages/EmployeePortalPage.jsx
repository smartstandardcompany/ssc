          ...
          <Card className="border-warning/50 bg-warning/5">
            ...
            <CardContent><div className="space-y-2">{pendingPayments.map(p => (
              <div key={p.id} className="flex justify-between items-center p-3 bg-background rounded-xl border">
                <div><div className="font-medium text-sm">${p.amount.toFixed(2)} - {(p.payment_type || 'salary').replace('_', ' ')}</div><div className="text-xs text-muted-foreground">{p.period} | {p.payment_mode}</div></div>
