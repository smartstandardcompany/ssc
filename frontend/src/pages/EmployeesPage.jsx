...
export default function EmployeesPage() {
  ...
  const [payData, setPayData] = useState({ amount: '', payment_mode: 'cash', branch_id: '', period: '', date: new Date().toISOString().split('T')[0], notes: '' });
  ...
  const handlePaySalary = async (e) => {
    ...
    try {
      ...
      setShowPayDialog(false); setPayData({ amount: '', payment_mode: 'cash', branch_id: '', period: '', date: new Date().toISOString().split('T')[0], notes: '' });
      fetchData();
      ...
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-muted-foreground">Manage employees, payroll & document tracking</p>
          </div>
              ...
              <DialogContent className="max-w-2xl" data-testid="employee-dialog">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div><Label>Salary</Label><Input type="number" step="0.01" value={formData.salary} data-testid="emp-salary" onChange={(e) => setFormData({ ...formData, salary: e.target.value })} /></div>
                    ...
                    <div><Label>Document Expiry Date</Label><Input type="date" value={formData.document_expiry} data-testid="emp-doc-expiry" onChange={(e) => setFormData({ ...formData, document_expiry: e.target.value })} /></div>
                  </div>
                      ...
                      <tr key={emp.id} className="border-b border-border hover:bg-secondary/50" data-testid="employee-row">
                        ...
                        <td className="p-3 text-right">
                          <div className="flex gap-1 justify-end">
                            <Button size="sm" variant="outline" onClick={() => { setPayingEmp(emp); setPayData({ ...payData, amount: emp.salary || '' }); setShowPayDialog(true); }} data-testid="pay-salary-btn" className="h-8"><DollarSign size={14} className="mr-1" />Pay</Button>
        ...
        {salaryPayments.length > 0 && (
          <Card className="border-border">
            <CardHeader><CardTitle className="font-outfit">Recent Salary Payments</CardTitle></CardHeader>
            <CardContent>
              <div className="overflow-x-auto">
                <table className="w-full" data-testid="salary-payments-table">
                  <thead><tr className="border-b border-border">
                  ...
                  <tbody>
                    {salaryPayments.slice(0, 20).map(p => (
                      <tr key={p.id} className="border-b border-border hover:bg-secondary/50">
                        ...
                        <td className="p-3 text-sm font-medium">{p.employee_name}</td>
                        <td className="p-3 text-sm">{p.period}</td>
                        <td className="p-3 text-sm text-right font-medium">${p.amount.toFixed(2)}</td>
                        <td className="p-3"><Badge variant="secondary" className="capitalize">{p.payment_mode}</Badge></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        <Dialog open={showPayDialog} onOpenChange={setShowPayDialog}>
          <DialogContent data-testid="pay-salary-dialog">
            <DialogHeader><DialogTitle className="font-outfit">Pay Salary - {payingEmp?.name}</DialogTitle></DialogHeader>
            <form onSubmit={handlePaySalary} className="space-y-4">
              ...
              <div className="flex gap-3">
                <Button type="submit" data-testid="submit-salary" className="rounded-full">Pay Salary</Button>
