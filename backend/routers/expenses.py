...
async def create_expense(expense_data: ExpenseCreate, current_user: User = Depends(get_current_user)):
    ...
    if data.get('supplier_id') and expense_data.payment_mode == "credit":
        supplier = await db.suppliers.find_one({"id": data['supplier_id'], **get_tenant_filter(current_user)}, {"_id": 0})
        if supplier:
    ...
    stamp_tenant(expense_dict, current_user)
    await db.expenses.insert_one(expense_dict)
    if data.get('supplier_id') and expense_data.payment_mode == "credit":
        supplier = await db.suppliers.find_one({"id": data['supplier_id'], **get_tenant_filter(current_user)}, {"_id": 0})
        if supplier:
            new_credit = supplier.get("current_credit", 0) + expense_data.amount
            await db.suppliers.update_one({"id": data['supplier_id'], **get_tenant_filter(current_user)}, {"$set": {"current_credit": new_credit}})
    # Log activity
...
async def delete_expense(expense_id: str, current_user: User = Depends(get_current_user)):
    require_permission(current_user, "expenses", "write")
    expense = await db.expenses.find_one({"id": expense_id, **get_tenant_filter(current_user)}, {"_id": 0})
    if not expense:
    ...
    if expense.get("supplier_id") and expense.get("payment_mode") == "credit":
        supplier = await db.suppliers.find_one({"id": expense["supplier_id"], **get_tenant_filter(current_user)}, {"_id": 0})
        if supplier:
    ...
    result = await db.expenses.delete_one({"id": expense_id, **get_tenant_filter(current_user)})
...
async def create_expense_refund(body: dict, current_user: User = Depends(get_current_user)):
    ...
    if original_expense_id:
        original = await db.expenses.find_one({"id": original_expense_id, **get_tenant_filter(current_user)}, {"_id": 0})
    ...
    stamp_tenant(refund, current_user)
    ...
    stamp_tenant(ne_dict, current_user)
    ...
    if refund.get("supplier_id") and refund_mode == "credit":
        supplier = await db.suppliers.find_one({"id": refund["supplier_id"], **get_tenant_filter(current_user)}, {"_id": 0})
        if supplier:
            new_credit = max(0, supplier.get("current_credit", 0) - amount)
            await db.suppliers.update_one({"id": refund["supplier_id"], **get_tenant_filter(current_user)}, {"$set": {"current_credit": new_credit}})
...
async def get_expense_refunds(current_user: User = Depends(get_current_user)):
    ...
    require_permission(current_user, "expenses", "read")
    refunds = await db.expense_refunds.find(get_tenant_filter(current_user), {"_id": 0}).sort("date", -1).to_list(1000)
    return refunds
...
async def auto_categorize_expense(body: dict, current_user: User = Depends(get_current_user)):
    ...
    categories = await db.categories.find(get_tenant_filter(current_user), {"_id": 0}).to_list(100)
...
async def get_capital_expenses(current_user: User = Depends(get_current_user)):
    return await db.capital_expenses.find(get_tenant_filter(current_user), {"_id": 0}).sort("date", -1).to_list(1000)
...
async def delete_capital_expense(cap_id: str, current_user: User = Depends(get_current_user)):
    await db.capital_expenses.delete_one({"id": cap_id, **get_tenant_filter(current_user)})
    return {"message": "Deleted"}

# Recurring Expenses
@router.get("/recurring-expenses")
async def get_recurring_expenses(current_user: User = Depends(get_current_user)):
    recs = await db.recurring_expenses.find(get_tenant_filter(current_user), {"_id": 0}).to_list(100)
...
async def delete_recurring_expense(rec_id: str, current_user: User = Depends(get_current_user)):
    await db.recurring_expenses.delete_one({"id": rec_id, **get_tenant_filter(current_user)})
...
async def renew_pay_recurring(rec_id: str, body: dict, current_user: User = Depends(get_current_user)):
    rec = await db.recurring_expenses.find_one({"id": rec_id, **get_tenant_filter(current_user)}, {"_id": 0})
    ...
    await db.recurring_expenses.update_one({"id": rec_id, **get_tenant_filter(current_user)}, {"$set": {"next_due_date": new_due.isoformat(), "amount": amount}})
