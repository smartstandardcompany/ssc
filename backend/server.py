...
async def analyze_statement(stmt_id: str, current_user: User = Depends(get_current_user)):
    ...
    for t in txns:
        ...
        if not key or len(key) < 3:
            ...
            clean = re_mod.sub(r'\d{10,}', '', desc)
            clean = re_mod.sub(r'\d{2}/\d{2}/\d{4}', '', clean)
        ...
        raw_senders[key]["total_debit"] += t.get("debit", 0)
        raw_senders[key]["last_date"] = t.get("date", "")
        raw_senders[key]["fees"] += t.get("transfer_fee", 0)
        raw_senders[key]["vat"] += t.get("transfer_vat", 0)
...
@api_router.post("/job-titles")
async def create_job_title(body: dict, current_user: User = Depends(get_current_user)):
    jt = JobTitle(
        title=body["title"],
        department=body.get("department", ""),
        min_salary=float(body.get("min_salary", 0)),
        max_salary=float(body.get("max_salary", 0)),
...
@api_router.put("/job-titles/{title_id}")
...
@api_router.delete("/job-titles/{title_id}")
...
@api_router.post("/stock/entries/bulk")
...
@api_router.post("/stock/usage/bulk")
...
Extract every line item. For Arabic text, translate item names to English. If quantity or price is unclear, make best estimate. Return ONLY the JSON, no markdown."""
        ...
        return {"raw_text": response, "items": [], "error": "Could not parse structured data"}
    except Exception as e:
...
@api_router.post("/shift-assignments/bulk")
...
@api_router.put("/shift-assignments/{assignment_id}")
...
@api_router.get("/shift-assignments/attendance-summary")
